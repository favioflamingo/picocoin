...
static void test_script(bool is_valid, cstring* scriptSig,
                        ...
                        const unsigned int test_flags, int64_t amount)
{
    struct bp_tx tx = BuildCreditingTransaction(scriptPubKey);
    tx = BuildSpendingTransaction(scriptSig, &tx);

    bool rc;
    rc = bp_script_verify_with_value(scriptSig, scriptPubKey, &tx, 0, test_flags, SIGHASH_NONE, amount);
...
static void runtest(const char *basefn)
{
	...
	for (idx = 0; idx < json_array_size(tests); idx++) {
		...
		if (json_array_size(test) > 1){
			// get array
			double amt = 0;
			if(json_is_array(json_array_get(test, pos)) ){
				int ify = 0;
				json_t *amts = json_array_get(test, pos);
				assert(json_array_size(amts) == 1);
				assert(json_is_real(json_array_get(amts, ify)));
				amt = json_real_value(json_array_get(amts, ify));
				ify++;
			...
			//fprintf(stderr,"i=%d: amt=%f and scriptSig=%s and scriptPub=%s\n",
			//		amt,idx,scriptSigEnc,scriptPubKeyEnc);
			...
			if (strlen(json_flags) > 0) {
				...
				do {
					...
					else if (strcmp(json_flag, "SIGHASH_FORKID") == 0)
			...
			// convert to satoshi
			int64_t amount = (int64_t) amt * 100000000;

			test_script(is_valid, scriptSig, scriptPubKey,
				    idx, scriptSigEnc, scriptPubKeyEnc, verify_flags, amount);
...
{
    runtest("data/uahf_script_tests.json");
    return 0;
}
